<script lang="ts">
	...
	const paymentData = (page.state as any).paymentData;
	...
	let memberSearchQuery = $state(paymentData.memberName || '');
	...
	async function submitForm() {
		...
		try {
			...
			// formDataToSend.append('payment_date', formatToYYYYMMDD(formData.paymentDate));

			const payload: Payment.Create = {
				amount: formData.amount,
				date: formatToYYYYMMDD(formData.paymentDate),
				payment_reference: formData.receiptNumber,
			...
			// if (formData.file) {
			// 	formDataToSend.append('receipt', formData.file);
			// }
		...
		<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
			...
			<div class="md:col-span-2">
				...
				{#if paymentData.photo !== ''}
					<ImageViewer src={paymentData.photo} alt="Payment Receipt" thumbnailSize="large" />
				{:else}
