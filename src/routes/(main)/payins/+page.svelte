<script lang="ts">
	...
	const fetchMemberList = async (search?: boolean, query?: string) => {
		try {
			...
			userList.set(response.data.data);
...
<div class="min-zh-screen text-black">
	{#if $userList.length}
		<div class="mb-4 flex items-center justify-between">
			...
			<button class="btn btn-primary" on:click={() => goto('/payins/create')}
				><Plus class="mr-2 h-4 w-4" />Add Payment</button
			>
		</div>
				...
				<tbody>
					{#each $userList as user, index}
						<tr>
							...
							<td>{user.amount}</td>
							<td>{user.date}</td>
							<td>{user.payment_mode.charAt(0).toUpperCase() + user.payment_mode.slice(1)}</td>
							<td>{user.reciept_number}</td>
						</tr>
					{/each}
				</tbody>
			</table>
