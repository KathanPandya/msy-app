<script lang="ts">
	...
	let paymentList = $state<Payment.Payout_List>([]);
	...
	async function fetchMemberAndPaymentList() {
		...
		try {
			...
			// const response = await paymentApi.getAllPayouts({
			// 	limit: 100,
			// 	page: 1
			// });
	...
	function goToUpdate(row: any) {
		const matchedPayout = paymentList.find((payment) => payment.deadMemberId == row.userId);
		goto(`/payouts/update/${row._id}`, {
			state: {
				payoutData: { ...row, ...matchedPayout }
			}
		});
	}
	...
	const tableData = $derived(
		paymentList.map((payment) => {
			console.log($memberListStore.members);
			const member = $memberListStore.members.find((user) => user._id === payment.deadMemberId);
			return {
...
<div class="flex h-full flex-col">
	...
	<div class="mb-4 flex-shrink-0">
		<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
			...
			<!-- Add Member Button -->
			<Button variant="primary" onclick={() => goto('/payouts/create')}>
		...
		{#if !isLoading && paymentList.length > 0}
			<p class="mt-4 text-sm text-gray-700">
				Showing <span class="font-medium">{paymentList.length}</span>
				{paymentList.length === 1 ? 'entry' : 'entries'}.
			</p>
		{/if}
	</div>
