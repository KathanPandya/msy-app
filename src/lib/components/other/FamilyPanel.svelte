<script lang="ts">
	...
	let { clubId }: { clubId: string | null } = $props();
	...
	async function load() {
		...
		errorMessage = '';
		try {
			const res = await familiesApi.detail(clubId);
			family = res.family;
		} catch (err: any) {
...
<div class="space-y-3">
	{#if !clubId}
		<div class="rounded-md bg-gray-50 p-4 text-sm text-gray-600">
			This member is not part of any family group.
		</div>
	{:else if isLoading}
			...
			{#each family.members as m (m.id)}
				...
				>
					<div class="flex items-start justify-between gap-2">
						...
						<span
							class="shrink-0 rounded-full px-2 py-0.5 text-[11px] font-medium {m.status === 'active'
